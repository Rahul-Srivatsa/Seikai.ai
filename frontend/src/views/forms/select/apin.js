...
const RealTimeTranscription = () => {
  ...
  const stopListening = async () => {
    ...
    if (transcript.trim() && count <= 3) {
      try {
        const response = await fetch('https://seikai-ai.onrender.com/api/gemini', {
          method: 'POST',
  ...
  const getfeedback = async () => {
    if (transcript.trim()) {
      try {
        const response = await fetch('https://seikai-ai.onrender.com/api/gemini/feedback', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  return (
    <div className="chat-bubble-container">
      ...
      <div className="speech-box">
        <p>
          {count >= 3 ? 'Feedback' : 'Question'}: {question}
        </p>
      </div>
